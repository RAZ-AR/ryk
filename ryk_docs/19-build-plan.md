...
1. ✅ Простой admin `/concierge`: добавление, снятие с показа и проверка `experiences`,
   ссылка на билеты. Доступ по `CONCIERGE_TOKEN`, без токена — 404 ([ADR-006](18-decisions-adr.md)).
2. ⏭️ Шаблоны AI-сообщений — **не нужны**: тексты компаньона уже живут кодами
   в `lib/engine/nudges.ts` + `messages/*.json` (Фаза 8). Это лучше шаблонов в БД:
   тон проверяется тестами и переводится вместе с интерфейсом.
3. ⏭️ Лог интервью после недели — **отложен до первых пользователей**: пока их нет,
   отдельная модель в БД была бы формой без содержания. Шаблон разговора —
   [20-interview-guide](20-interview-guide.md), записи ведём вне продукта.

**✅ Gate пройден:** куратор завёл «Джазовый вечер в Дому омладине» → событие стало
кандидатом №1 у реального пользователя; «Снять» убирает его из кандидатов, «Вернуть» —
возвращает; событие с прошедшей датой отсеивается само.
